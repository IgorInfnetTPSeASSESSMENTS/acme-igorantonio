...
import { db } from '../infra/firebase'; // Certifique-se de importar o Firestore
...
function Login() {
  ...
  const handleLogin = async () => {
    try {
      ...
      // Redirecionar ou realizar outras ações necessárias após login bem-sucedido
    } catch (error) {
  ...
  return (
    ...
    >
      <Box sx={{ maxWidth: 400, width: '100%', display: 'flex', flexDirection: 'column' }}>
        ...
        {error && (
          <TypographyComponent variant="p" color="red" sx={{ mb: 2, textAlign: 'center', marginTop: '20px', fontWeight: 'bold', fontSize: '20px' }}>
            {error}
          </TypographyComponent>
        )}
      </Box>
